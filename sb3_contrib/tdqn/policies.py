...
from sb3_contrib.common.torch_layers import create_noisy_mlp
from torch import nn


class QuantileNetwork(BasePolicy):
    """
    Quantile network for TDQN
    ...
    def __init__(
        ...
        quantile_net = create_noisy_mlp(self.features_dim, 0, self.net_arch, self.activation_fn)
        self.quantile_net = nn.Sequential(*quantile_net)

        # add action branches
        self.value_head = nn.Linear(hidden_dim, 1)
        self.adv_heads = nn.ModuleList([nn.Linear(hidden_dim, n_quantiles) for _ in range(action_dim)])
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        ...
        """
        features = self.quantile_net(self.extract_features(obs))
        state_value = self.value_head(features)
        action_adv = th.stack([branch(features) for branch in self.adv_heads], dim=1)
        q_value = state_value.unsqueeze(2) + action_adv - action_adv.mean(2, keepdim=True)
        return q_value.permute(0, 2, 1)
...
class TDQNPolicy(BasePolicy):
    """
    Policy class with quantile and target networks for TDQN.
    ...
    def __init__(
        ...
        super(TDQNPolicy, self).__init__(
            observation_space,
            action_space,
